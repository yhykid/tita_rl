...
class SingleStand(BaseTask):
    ...
    def _reward_stand_up(self):
        ...
        upright = torch.tensor([0., 0., 1.], device=self.device).repeat(self.num_envs, 1)
        upright_vec = quat_apply_yaw(self.base_quat, upright)
        cosine_dist = torch.sum(forward * upright_vec, dim=-1) / torch.norm(upright_vec, dim=-1)
        return torch.square(0.5*cosine_dist+0.5)

    def _reward_single_contact(self):
        if_right_contact = torch.norm(self.contact_forces[:, self.feet_indices[0], :], dim=-1) > 1e-5
        if_left_contact = torch.norm(self.contact_forces[:, self.feet_indices[1], :], dim=-1) > 1e-5
        #print(int(if_right_contact * ~if_left_contact))
        return if_right_contact * ~if_left_contact


    def _reward_raise_foot(self):
        foot_height = self.rigid_body_states[:, self.feet_indices[1],2]
        error = torch.abs(foot_height-0.4)
        return torch.exp(-error/self.cfg.rewards.tracking_sigma)
    
    def _reward_penal(self):
        if_left_contact = torch.norm(self.contact_forces[:, self.feet_indices[1], :], dim=-1) > 1e-5
        reward = torch.zeros_like(self.projected_gravity[:,2])
        reward[if_left_contact] = 1.0
        return reward
